-- ============================================================================
-- Faz 1 · Yerleştirici artık reddetmiyor
-- ============================================================================
-- Kural (K5): admin hiçbir işlemde hata almamalı. Bugün iki yerde duruyordu:
--
--   1. free_lesson_slots istenen sayıda boş slot bulamazsa eksik döndürüyor,
--      çağıran da "Ders programında yeterli boş slot bulunamadı" diyerek
--      işlemi hiç yapmıyordu.
--   2. rpc_apply_chain_dates, hedef saatte başka bir öğrencinin dersi ya da
--      bir deneme dersi varsa "çakışma" hatasıyla dönüyor, tek bir satır bile
--      yazmıyordu.
--
-- İkisi de kalkıyor. Çakışma artık bir hata değil, yazılan ve raporlanan bir
-- durum: admin programda görüp karar veriyor.
--
-- Korunan tek sert kural: aynı öğrencinin aynı anda iki dersi olamaz. Bu bir
-- iş kararı değil, fiziksel imkânsızlık — hem yerleştiricide hem de
-- uniq_active_planned_slot / prevent_duplicate_lesson_instance ile duruyor.

-- ─────────────────────────────────────────────────────────────────────────
-- 1) İki geçişli yerleştirici
-- ─────────────────────────────────────────────────────────────────────────
-- 1. geçiş: bugünkü davranış — başkasının dersi ya da deneme dersi olan
--    slotları atla.
-- 2. geçiş: hâlâ eksik varsa aynı takvimi yeniden yürü, bu kez yalnızca
--    öğrencinin kendi dersiyle dolu slotları atla. Yani "hiç yer yok" diye
--    boş dönmek yerine, şablon saatine yerleştirip çakışmayı kabul et.
--
-- Pratikte 2. geçiş nadiren çalışır (400 günlük pencerede haftalık şablon
-- yüzlerce aday üretir); bir emniyet supabı, olağan yol değil.
...
DECLARE
  ...
  v_taken    boolean;
  v_anahtar  text;
  v_verilen  text[] := '{}';   -- bu çağrıda çoktan döndürülen (gün|saat) çiftleri
BEGIN
  ...
  IF v_slots IS NULL THEN
    RETURN;  -- şablon yok: yerleştirilecek bir kalıp da yok
  END IF;
  ...
  FOR v_pass IN 1..2 LOOP
    ...
    WHILE v_offset <= 400 AND v_found < p_count LOOP
      ...
      LOOP
        ...
        -- 2. geçişte 1. geçişin verdiği slotu tekrar verme.
        v_anahtar := v_day::text || '|' || (v_slot->>'s');
        CONTINUE WHEN v_anahtar = ANY (v_verilen);

        -- Her iki geçişte de geçerli sert kural: öğrencinin kendi dersiyle
        -- dolu slot. Öğretmeni fark etmez — öğrenci aynı anda iki yerde olamaz.
        SELECT EXISTS (
          SELECT 1
        ...
        IF v_pass = 1 THEN
          -- Öğretmenin başka bir öğrencisi bu slotta mı? (Arşivli öğrenciler
          -- sayılmaz: eski kayıtları bakiye bütünlüğü için duruyor, yeni
          -- programı engellememeli.)
          SELECT EXISTS (
            SELECT 1
            FROM lesson_instances li
...
COMMENT ON FUNCTION public.free_lesson_slots(uuid, uuid, integer, date, time, boolean, uuid[], date) IS
  'Şablon slotlarında ileri yürüyerek yer bulur. 1. geçiş boş slot arar; yetmezse 2. geçiş yalnızca öğrencinin kendi dersini atlayarak yerleştirir, böylece çağıran hiçbir zaman "yer yok" diye başarısız olmaz.';

-- ─────────────────────────────────────────────────────────────────────────
-- 2) Çakışma reddi → uyarı
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Saati verilmeyen satır "yalnızca tarihi değiştir" demek: o gündeki ilk
  -- uygun şablon slotunu al, gün şablonda yoksa dersin kendi saatinde kal.
  SELECT jsonb_agg(
           CASE
  ...
  -- Sert kural: aynı öğrenciye aynı tarih ve saatte iki ders yazılamaz.
  -- Bu bir iş kararı değil, fiziksel imkânsızlık; tek kalan reddetme noktası.
  IF EXISTS (
  ...
  -- Başka öğrencinin dersi ya da deneme dersi: artık engel değil, uyarı.
  -- Admin programda kırmızı şeridi görüp karar verir (K5).
  WITH cakisan AS (
    SELECT DISTINCT
           (e->>'lessonDate')::date AS tarih,
           to_char((e->>'startTime')::time, 'HH24:MI') || ' - ' ||
           to_char((e->>'endTime')::time,   'HH24:MI') AS saat,
           COALESCE(pr.full_name, 'Bilinmeyen Öğrenci') AS kim
    ...
    SELECT DISTINCT
           (e->>'lessonDate')::date,
           to_char((e->>'startTime')::time, 'HH24:MI') || ' - ' ||
           to_char((e->>'endTime')::time,   'HH24:MI'),
           'Deneme Dersi'
      FROM jsonb_array_elements(v_resolved) e
  ...
  -- Dönen zincirin uniq_active_planned_slot'u cümle ortasında ihlal etmemesi
  -- için taşınan satırları önce benzersiz sahte tarihlere park et.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 3) "Yeterli boş slot bulunamadı" hatalarının kalan izleri
-- ─────────────────────────────────────────────────────────────────────────
-- Yerleştirici artık eksik dönmüyor; bu dallara yalnızca öğrencinin şablonu
-- hiç yoksa girilir. Mesaj da onu söylesin.
...
BEGIN
  ...
  IF v_updates IS NULL OR jsonb_array_length(v_updates) <> v_needed THEN
    RETURN json_build_object('success', false, 'error',
      'Öğrencinin haftalık ders programı tanımlı değil, dersler yerleştirilemedi.');
  END IF;
...
-- ─────────────────────────────────────────────────────────────────────────
-- 4) "İşlenmiş bir ders ertelenemez" reddi admin için kalkıyor
-- ─────────────────────────────────────────────────────────────────────────
-- K5: admin geçmişteki bir dersi de taşıyabilmeli. Öğretmen zaten bu işlemi
-- hiçbir ekrandan çağırmıyor; yine de sınır orada dursun.
...
BEGIN
  ...
  -- Tıklanan ders her hâlükârda taşınır; elle sabitlenmiş dersler yerinde
  -- kalır ve kaydırma onların etrafından akar.
