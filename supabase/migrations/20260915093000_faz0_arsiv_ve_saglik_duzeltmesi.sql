-- ============================================================================
-- Faz 0 · Arşivleme kuralı ve sağlık ölçütü düzeltmesi
-- ============================================================================
-- Arşivleme, planlı dersleri yalnızca arşivi yapan öğretmenin eşleşmesinde
-- siliyordu. Öğrenci daha önce başka bir öğretmendeyse o eşleşmedeki planlı
-- dersler geride kalıyor ve eski öğretmenin takviminde slot tutmaya devam
-- ediyordu (İpek · 19 Haziran'da arşivlendi, üç dersi Fatih'te kaldı).
--
-- İşlenmiş derslere ve bakiyeye dokunulmuyor: ders yapıldıysa ücreti
-- öğretmenindir, öğrencinin arşivlenmesi bunu geri almaz.

CREATE OR REPLACE FUNCTION public.rpc_archive_student_impl(
...
BEGIN
  ...
  -- Öğretmen eşleşmesine bakılmıyor: öğrenci arşivlendiyse hiçbir takvimde
  -- planlı dersi kalmamalı.
...
-- Sağlık ölçütü aynı körlüğe sahipti: arşivli öğrencinin planlı dersini
-- yalnızca güncel öğretmen eşleşmesinde arıyordu. Ayrıca iki ölçütün ağırlığı
-- düzeltildi: silinmiş öğretmene bağlı işlenmiş dersler ve tarihi geçmiş
-- işaretlenmemiş dersler birer "kritik" değil, adminin karar vermesi gereken
-- "orta" kalemler.
...
AS $fn$
  SELECT * FROM (
    ...
    UNION ALL
    SELECT 'panelde_yabanci_ders', 'Öğrenci panelinde başka öğretmenin dersi', count(*)::bigint, 'kritik'
      FROM students s
    ...
    UNION ALL
    SELECT 'deneme_cakismasi', 'Bugün/sonrası ders–deneme çakışması', count(*)::bigint, 'orta'
      FROM lesson_instances li JOIN trial_lessons tl
    ...
    UNION ALL
    -- Öğretmen eşleşmesi aranmıyor: arşivli öğrencinin hangi takvimde olursa
    -- olsun planlı dersi olmamalı.
    ...
    SELECT 'tekrarli_ders_numarasi', 'Aynı döngüde tekrar eden ders numarası', coalesce(sum(x.fazla),0)::bigint, 'dusuk'
      FROM (SELECT count(*) - 1 AS fazla FROM lesson_instances WHERE status IN ('planned','completed')
