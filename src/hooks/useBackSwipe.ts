...
export function useBackSwipe<T extends HTMLElement>(): {
  ...
  const resetTransform = useCallback((animated: boolean) => {
    ...
    if (content) {
      if (animated) {
        content.style.transition = `transform ${CANCEL_DURATION}ms cubic-bezier(0.2, 0, 0, 1), box-shadow ${CANCEL_DURATION}ms ease-out`;
      }
    ...
    if (scrim) {
      if (animated) {
        ...
        const cleanup = () => {
          ...
          scrim.removeEventListener('transitionend', cleanup);
        };
  ...
  const completeTransition = useCallback(() => {
    ...
    if (content) {
      content.style.transition = `transform ${COMPLETE_DURATION}ms cubic-bezier(0.2, 0, 0, 1), box-shadow ${COMPLETE_DURATION}ms ease-out`;
